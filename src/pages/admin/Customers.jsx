...
export default function Customers() {
  ...
  // 1. Fetch live customers and aggregate their order totals & history
  ...
  const toggleBlockMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (viewCustomer) {
        setViewCustomer((/** @type {any} */ prev) => prev ? ({ ...prev, status: prev.status === 'blocked' ? 'active' : 'blocked' }) : null);
      }
    },
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredCustomers.map((/** @type {any} */ c) => {
      return [
        ...
        formatDateTime(c.created_at), c.totalOrders, c.totalSpend, c.totalSpend * KHR_RATE,
  ...
  return (
    <div className="w-full bg-white rounded-md shadow-sm border border-slate-200 relative flex flex-col h-[calc(100vh-8rem)]">
            ...
            <div className="p-6 text-center">
              ...
              <div className="flex justify-center gap-3">
                ...
                <button onClick={() => deleteMutation.mutate(deleteCustomer.id)} disabled={deleteMutation.isPending} className="px-5 py-2.5 text-sm font-medium bg-rose-600 text-white rounded-lg hover:bg-rose-700 transition-colors w-full flex items-center justify-center gap-2">
                  {deleteMutation.isPending ? <RefreshCcw size={16} className="animate-spin" /> : null}
      ...
      {/* 2. Block/Unblock Customer Modal */}
      {blockCustomer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              ...
              <h2 className="text-xl font-bold text-slate-900 mb-2">
                {blockCustomer.status === 'blocked' ? 'Unblock Customer?' : 'Block Customer?'}
              </h2>
              ...
              <div className="flex justify-center gap-3">
                ...
                <button 
                  onClick={() => toggleBlockMutation.mutate({ id: blockCustomer.id, currentStatus: blockCustomer.status })} 
                  disabled={toggleBlockMutation.isPending} 
                  className={`px-5 py-2.5 text-sm font-medium text-white rounded-lg transition-colors w-full flex items-center justify-center gap-2 ${blockCustomer.status === 'blocked' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-amber-600 hover:bg-amber-700'}`}
                >
                  {toggleBlockMutation.isPending ? <RefreshCcw size={16} className="animate-spin" /> : null}
