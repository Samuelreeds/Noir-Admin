...
import { Save, Image as ImageIcon, Layout, Info, Phone, RefreshCcw, CheckCircle, Filter, Truck, X, UploadCloud, Search, Plus, Edit, Trash2 } from 'lucide-react';
...
// --- REUSABLE TOGGLE SWITCH (FIXED CSS OVERLAP) ---
/**
...
export default function WebSetup() {
  ...
  return (
    <div className="w-full h-[calc(100vh-5rem)] bg-white border border-slate-200 rounded-lg shadow-sm flex flex-col overflow-hidden">
      
      {/* GLOBAL TOP HEADER */}
      <div className="px-6 py-4 border-b border-slate-200 bg-white flex justify-between items-center shrink-0">
        <h1 className="text-xl font-bold text-slate-800 tracking-wide uppercase">WEB SETUP</h1>
      </div>

      <div className="flex flex-1 overflow-hidden">
        
        {/* LEFT SIDEBAR NAVIGATION */}
        <div className="w-64 bg-slate-50/50 border-r border-slate-200 p-4 space-y-1 shrink-0 overflow-y-auto">
          <button onClick={() => { setActiveTab('hero'); setSliderView('list'); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'hero' ? 'bg-slate-100/80 text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-slate-100/50 hover:text-slate-900'}`}><Layout size={18} /> Slider Menu</button>
          <button onClick={() => setActiveTab('about')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'about' ? 'bg-slate-100/80 text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-slate-100/50 hover:text-slate-900'}`}><Info size={18} /> About Section</button>
          <button onClick={() => setActiveTab('contact')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'contact' ? 'bg-slate-100/80 text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-slate-100/50 hover:text-slate-900'}`}><Phone size={18} /> Footer & Contact</button>
          <button onClick={() => setActiveTab('filters')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'filters' ? 'bg-slate-100/80 text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-slate-100/50 hover:text-slate-900'}`}><Filter size={18} /> Store Filters</button>
          <button onClick={() => setActiveTab('shipping')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'shipping' ? 'bg-slate-100/80 text-slate-900 shadow-sm' : 'text-slate-500 hover:bg-slate-100/50 hover:text-slate-900'}`}><Truck size={18} /> Shipping & Tax</button>
        </div>

        {/* MAIN CONTENT AREA */}
        <div className="flex-1 bg-slate-50 overflow-y-auto custom-scrollbar p-6 md:p-8">
          ...
          {/* ======================================= */}
          {/* 2. STORE SETTINGS TABS (ABOUT, ETC)     */}
          {/* ======================================= */}
          {activeTab !== 'hero' && (
